...
class Rob6323Go2Env(DirectRLEnv):
    ...
    def __init__(self, cfg: Rob6323Go2EnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.last_actions = torch.zeros(self.num_envs, gym.spaces.flatdim(self.single_action_space), 3, dtype=torch.float, device=self.device, requires_grad=False)     # <--- Added
        ...
        # </--- Added for Advanced Foot Interaction
    ...
    def _apply_action(self) -> None:
        # self.robot.set_joint_position_target(self._processed_actions) # <--- Removed
        # </--- Added
        # Compute PD torques
        torques = torch.clip(
            (
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # </--- Added for foot clearance reward
        phases = 1 - torch.abs(1.0 - torch.clip((self.foot_indices * 2.0) - 1.0, 0.0, 1.0) * 2.0)
        foot_height = (self.foot_positions_w[:, :, 2]).view(self.num_envs, -1) # <--- Added feet heights
        target_height = 0.08 * phases + 0.02 # offset for foot radius 2cm
        rew_foot_clearance = torch.sum(torch.square(target_height - foot_height) * (1 - self.desired_contact_states), dim=1)  # <--- Added feet clearance reward
        # --- Added />

        # </--- Added for Contact Forces
        ...
        # Feet air time reward  --- scale is 0, ignore

        # Penalty for having more or less than 2 feet in contact
        rew_foot2contact = - torch.abs((self._contact_sensor.data.net_forces_w[:, self._feet_ids_sensor, 2] > 1.0).sum(1) - 2) / 2
        ...
        rewards = {
            ...
            "rew_foot2contact": rew_foot2contact * self.cfg.foot2contact_reward_scale
        }
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        self.last_actions[env_ids] = 0.     # <--- Added

        # Reset raibert quantity
        self.gait_indices[env_ids] = 0      # <--- Added
    ...
    @property
    def foot_positions_w(self) -> torch.Tensor:     # <--- Added
    ...
    def _step_contact_targets(self):    # <--- Added
        frequencies = 3.
        phases = 0.5
        offsets = 0.
        bounds = 0.
    ...
    def _reward_raibert_heuristic(self):    # <--- Added
